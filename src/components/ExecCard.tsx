...
const ExecCard = ({ name, role, link, headshot }: Props) => {
  return (
    <div className="w-full h-[520px] max-w-sm bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-900 dark:border-gray-700 backdrop-blur-lg transition-transform duration-300 hover:scale-105">
      <div className="flex flex-col items-center h-full pb-10">
        <img
          className="mb-3 shadow-lg h-[70%] w-full object-cover object-top rounded-t-lg"
          src={`${headshot}`}
          alt={`${name} image`}
        />
